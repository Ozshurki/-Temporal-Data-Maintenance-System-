...
def check_last_modified_database(last_modified_cursor, first_name, last_name, loinc_num, valid_date, start_date,
                                 end_date, long_common_name):
    query = " SELECT * " \
            " FROM last_modified " \
            " WHERE first_name = %s " \
            " AND loinc_num = %s " \
    ...
    for record in records:
        print(
            f"{first_name} {last_name} loinc_num: {record['loinc_num']} ({long_common_name}) with value: {record['value']} last modified at {record['modified_date']}")


def find_all_exams(is_with_time):
    query = " SELECT *" \
            " FROM PATIENTS " \
            " WHERE first_name = %s " \
...
def history_query(patients_cursor, inc_cursor, last_modified_cursor):
    ...
    try:
        ...
        query = find_all_exams(valid_date_with_time)
        ...
        for record in records:
            ...
            if record['last_modified']:
                check_last_modified_database(last_modified_cursor, first_name, last_name, loinc_num, date, start, end,
                                             long_common_name)
            else:
                print(
                    f"{first_name} {last_name} loinc_num: {record['loinc_num']} ({long_common_name}) with value: {record['value']} last modified at {record['transaction_time']}")
